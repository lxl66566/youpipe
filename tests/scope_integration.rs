...
fn test_scope_par_via_collect() {
    ...
    let offset = 100i32;
    let result = youpipe::scope(|s| {
        s.pipe(0..50i32).map(|x: i32| x + offset).collect()
    });
...
fn test_scope_empty() {
    let result = youpipe::scope(|s| {
        s.pipe(Vec::<i32>::new()).map(|x: i32| x * 2).collect()
    });
...
fn test_scope_large() {
    let factor = 2u64;
    let result = youpipe::scope(|s| {
        s.pipe(0..1000u64).map(|x: u64| x * factor).collect()
    });
    let mut r = result;
    r.sort_unstable();
...
fn test_scope_shared_lookup_across_pipelines() {
    let table: Vec<u64> = (0..1000).map(|i| (i as u64).wrapping_mul(2654435761)).collect();
